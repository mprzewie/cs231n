...
def affine_backward(dout, cache):
    ...
    N = x.shape[0]
    x_reshaped = x.reshape(N, -1)
    dx_reshaped, dw, db = np.zeros_like(x_reshaped), np.zeros_like(w), np.zeros_like(b)
...
def batchnorm_backward(dout, cache):
    ...
    dvar = (dx_norm * (x - r_mean) * (-1 / 2) * ((r_var + eps) ** (-3 / 2))).sum(axis=0)

    dmean = (dx_norm * (-1 / np.sqrt(r_var + eps))).sum(axis=0) + dvar * (((-2) * (x - r_mean)).mean(axis=0))

    dx = dx_norm * (1 / np.sqrt(r_var + eps)) + dvar * 2 * ((x - r_mean) / m) + dmean * (1 / m)
    #####################################
    # ######################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def batchnorm_backward_alt(dout, cache):
    ...
    # dx_norm = dout * gamma
    #
    # # dx = dx_norm * (1 / np.sqrt(r_var + eps)) + (dx_norm * (-1) * (x_norm ** (3/2))).mean(axis=0) +
...
def max_pool_forward_naive(x, pool_param):
    ...
    strides_x =  int(x.shape[2] / stride)
    strides_y = int(x.shape[3] / stride)
    out = np.zeros((x.shape[0], x.shape[1], strides_x, strides_y))
    ###########################################################################
    for s_x in range(strides_x):
        for s_y in range(strides_y):
            input = x[:, :, (s_x * pool_height):((s_x + 1) * pool_height), (s_y * pool_width):((s_y + 1) * pool_width)]
            # print(input)
            # print(input.max(axis=3))
            # print(input.max(axis=3).max(axis=2))
            # print('----')
            out[:, :, s_x, s_y] = input.max(axis=3).max(axis=2)
    ###########################################################################
    pass
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def max_pool_backward_naive(dout, cache):
    ...
    for s_x in range(strides_x):
        for s_y in range(strides_y):
            input = x[:, :, (s_x * pool_height):((s_x + 1) * pool_height), (s_y * pool_width):((s_y + 1) * pool_width)]
            the_dout = dout[:, :, s_x, s_y]
            maxes = input.max(axis=3).max(axis=2)
            mask = ((input.T / maxes.T) == 1).T.astype(int)
            dx[:, :, (s_x * pool_height):((s_x + 1) * pool_height), (s_y * pool_width):((s_y + 1) * pool_width)] += (mask.T * the_dout.T).T
